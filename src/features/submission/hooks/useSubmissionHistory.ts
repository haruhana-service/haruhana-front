    ...
    queryFn: async () => {
      ...
      const promises = days.map(async (day) => {
        try {
          ...
          const problems = await getDailyProblem(dateStr)
          return { date: dateStr, problems }
        } catch {
          // 해당 날짜에 문제가 없으면 빈 배열 반환
          return { date: format(day, 'yyyy-MM-dd'), problems: [] }
        }
      })
      ...
      results.forEach(({ date, problems }) => {
        problemsMap.set(date, problems)
      })
